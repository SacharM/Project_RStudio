...
````{r}
temp <- list.files ("C:/Users/imame/Desktop/Data Analyst/Case Study 1", full.names = TRUE, pattern = "\\.csv$")
...
### Order days of the week and order months
````{r}
...
ggplot(viz_1, aes(x=day_of_week, y=num_of_rides, fill=member_casual))+
  geom_bar(stat='identity', position = "dodge")+ labs(title = "Number of Rides by Day",
  x="Weekdays", y="Number of Rides")+ theme(axis.text.x = element_text(angle = 45))
````
...
ggplot(viz_2,aes(x=month,y=num_of_rides, fill=member_casual))+
  geom_bar(stat = 'identity', position = 'dodge')+ labs(title = 'Number of Rides by Month',
              x= 'Month', y='Number of Rides')+theme(axis.text.x = element_text(angle = 45, vjust = 0.6),
...
scatterplot %>%
  ...
  geom_point()+
  facet_wrap(~month)+theme(axis.text.x = 
element_text(angle = 45,vjust = 0.8), legend.title =        element_blank())+labs(title = "Average Ride Length for Each Month",  y="Ride Length",x="Weekdays")
```` 
